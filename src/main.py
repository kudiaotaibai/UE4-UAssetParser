...
def main():
    ...
    parser.add_argument(
        ...
        help="Output the Robot-style Blueprint component structure only (default)"
    )
    parser.add_argument(
        "--full-json", action="store_true",
        help="Output the full package/name/import/export/objectTree JSON"
    )

    args = parser.parse_args()
    raw_limit = None if args.full_raw else max(0, args.raw_limit)
    blueprint_only = (
        args.blueprint_only or (
            not args.full_json
