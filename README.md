...
`UAssetParser` 是一个用于解析 UE4.22 `.uasset` 文件的命令行工具，主要面向蓝图、UMG 和常规资源。工具会读取资源包信息、名称表、导入表、导出表和对象层级，并导出为 JSON，方便在不打开 UE4 编辑器的情况下查看资源结构。
...
默认情况下，工具会在输入资源文件旁边生成同名 JSON 文件。蓝图资源默认输出规整的组件结构 JSON，例如：
...
UAssetParser.exe BP_Character.uasset --full-json -o output\BP_Character_full.json
...
- `--blueprint-only`：只输出蓝图组件结构，输出为纯组件 JSON，保留 `Class`、`Parent`、`Socket`、`IsRoot` 和 `Properties`。这是默认行为。
- `--full-json`：输出旧版完整 JSON，包括包摘要、名称表、导入表、导出表和对象树。
...
```json
{
  ...
  "bp_DamageOverlapHeadMetal": {
    "Class": "SphereComponent",
    "Parent": "bp_SkelMesh",
    "Socket": "Head"
  },
  "bp_DamageOverlapBodyMetal": {
    "Class": "CapsuleComponent",
    "Parent": "bp_SkelMesh",
    "Socket": "Spine2"
  },
...
```text
output\demo_blueprints
```

这里的文件名都保持为普通 `.json`，不会带额外后缀。
...
python tests\verify_blueprint_variables.py
python tests\verify_socket_blueprints.py
python tests\verify_demo_blueprints_robot_shape.py
```
